export const NS_NAV_LEGACY = "nav-legacy";

export enum K {
  NAV_LEGACY = "NAV_LEGACY",
  LOGOUT = "LOGOUT",
  LOGIN = "LOGIN",
  PAGE_NOT_FOUND_TITLE = "PAGE_NOT_FOUND_TITLE",
  PAGE_NOT_FOUND_DESC = "PAGE_NOT_FOUND_DESC",
  PAGE_ERROR_TITLE = "PAGE_ERROR_TITLE",
  ACCOUNT_MANAGEMENT = "ACCOUNT_MANAGEMENT",
  BRICK_ERROR = "BRICK_ERROR",
  LEGACY_TEMPLATE_ERROR = "LEGACY_TEMPLATE_ERROR",
  HELP = "HELP",
  CANCEL = "CANCEL",
  OK = "OK",
  COVERT_TO_LANGUAGE = "COVERT_TO_LANGUAGE",
  SWITCH_THEME = "SWITCH_THEME",
  LIGHT_THEME = "LIGHT_THEME",
  DARK_THEME = "DARK_THEME",
  SEARCH_BY_NAME_KEYWORD = "SEARCH_BY_NAME_KEYWORD",
  FORBIDDEN = "FORBIDDEN",
  NOT_FOUND = "NOT_FOUND",
  SERVER_ERROR = "SERVER_ERROR",
  QUICK_ACCESS = "QUICK_ACCESS",
  NO_VISIT_ACCESS = "NO_VISIT_ACCESS",
  RECOMMEND_SEARCH = "RECOMMEND_SEARCH",
  SEARCH_RESULT_EMPTY = "SEARCH_RESULT_EMPTY",
  ENTER_KEY_WORDS = "ENTER_KEY_WORDS",
  ADD_TO_QUICK_ACCESS = "ADD_TO_QUICK_ACCESS",
  REMOVE_FROM_QUICK_ACCESS = "REMOVE_FROM_QUICK_ACCESS",
  SITE_MAP_SEARCH_PLACEHOLDER = "SITE_MAP_SEARCH_PLACEHOLDER",
  ALL_RESOURCES = "ALL_RESOURCES",
  MY_FAVORITE = "MY_FAVORITE",
  SEARCH_BY_NAME_OR_ID = "SEARCH_BY_NAME_OR_ID",
  IT_RESOURCE_MANAGEMENT = "IT_RESOURCE_MANAGEMENT",
  NO_MODEL = "NO_MODEL",
  RESOURCES = "RESOURCES",
  VIEW_ALL = "VIEW_ALL",
  SWITCH_ORG = "SWITCH_ORG",
  SWITCH_ORG_SUCCESS = "SWITCH_ORG_SUCCESS",
  LICENSE_EXPIRES_IN_DAY = "LICENSE_EXPIRES_IN_DAY",
  PAGE_RENDER_SLOW_TIP = "PAGE_RENDER_SLOW_TIP",
  VIEW_SUGGESTION = "VIEW_SUGGESTION",
  LICENSE_EXPIRATION_NOTICE = "LICENSE_EXPIRATION_NOTICE",
  LICENSE_EXPIRATION_DESC_PREFIX = "LICENSE_EXPIRATION_DESC_PREFIX",
  LICENSE_EXPIRATION_DESC_SUFFIX = "LICENSE_EXPIRATION_DESC_SUFFIX",
  OR_CLICK = "OR_CLICK",
  APPLY_FOR_EXTENSION = "APPLY_FOR_EXTENSION",
  DO_NOT_REMIND_AGAIN = "DO_NOT_REMIND_AGAIN",
  EXTENSION_APPLIED = "EXTENSION_APPLIED",
}

export type Locale = { [key in K]: string } & {
  [key in K as `${key}_other`]?: string;
} & {
  [key in K as `${key}_plural`]?: string;
};
